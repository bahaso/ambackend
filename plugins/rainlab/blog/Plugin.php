            ...
            'blog' => [
                ...
                'sideMenu' => [
                    ...
                    'news' => [
                        ...
                        'url'         => Backend::url('rainlab/blog/news'),
                        'permissions' => ['rainlab.blog.access_posts']
                    ],
                    'galleries' => [
                        'label'       => 'Photo Gallery',
                        'icon'        => 'icon-image',
                        'url'         => Backend::url('rainlab/blog/galleries'),
                        'permissions' => ['rainlab.blog.access_posts']
                    ]
                    // 'categories' => [
                    //     'label'       => 'rainlab.blog::lang.blog.categories',
                    //     'icon'        => 'icon-list-ul',
                    //     'url'         => Backend::url('rainlab/blog/calculhmac(clent, data)ategories'),
    ...
    public function boot()
    {
        ...
        Event::listen('backend.form.extendFields', function ($form) {
            foreach ($form->getFields() as $field) {
                if ( ! empty( $field->config['type'] ) && str_contains($field->config['type'], 'partial')) {
                    $form->addJs('/plugins/am/plugins/assets/js/froala.plugins.js', 'Am.Plugin');
                    break;
                }
            }
        });
